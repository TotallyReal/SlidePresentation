...
public class Parameter implements java.io.Serializable {
    ...
    /**
     * Returns the parameter for the given parameter name.The name should
     * include the path to the parameter separated by dots.
     *
     * For example, if there is a radius parameter inside the category param1,
     * then the full parameter name should be param1.radius. In case this
     * parameter doesn't exist, create it and set its value to be defObj.
     *
     * @param paramName
     * @param defObj
     * @return
    ...
    static ParameterToValueConverter strToColor = new ParameterToValueConverter<Color>() {

        @Override
        public Color convert(String repString) {
            String[] split = repString.split(",");
            return new Color(
    ...
    static ParameterType[] pTypes = new ParameterType[]{
        new ParameterType<String>("String", String.class, str -> str, str -> str),
        new ParameterType<Boolean>("Boolean", Boolean.class,
        value -> value.equals("true"), b -> b ? "true" : "false"),
        new ParameterType<Integer>("Integer", Integer.class,
        value -> Integer.parseInt(value), n -> "" + n),
        new ParameterType<Double>("Double", Double.class,
        value -> Double.parseDouble(value), d -> "" + d),
        new ParameterType<javafx.scene.paint.Color>("Color",
        javafx.scene.paint.Color.class,
        strToColor,
        color -> color.getRed() + "," + color.getGreen() + "," + color.getBlue() + "," + color.getOpacity()
        )
    };
    ...
    public static String objectType(Object obj) {
        ...
        for (ParameterType pType : pTypes) {
            if (pType.cls.isAssignableFrom(obj.getClass())) {
                return pType.name;
            }
        }
    ...
    private static Object stringToValue(String repStr, String type) {
        for (ParameterType pType : pTypes) {
            if (pType.name.equals(type)) {
                return pType.convertToValue(repStr);
            }
        }
        switch (type) {
            case "Boolean":
                return repStr.equals("true");
            case "Integer":
                return Integer.parseInt(repStr);
            case "Double":
                return Double.parseDouble(repStr);
            default:
                return repStr;
        }
    }
    ...
    private String getRepString(String initial) {
        ...
        for (ParameterType pType : pTypes) {
            if (pType.name.equals(type)) {
                val = pType.convertToString(getValue());
                break;
            }
        }
        return initial + type + "," + getName() + "=" + val + "\n";

    }
    
    // TO DO - change type to ParameterType
