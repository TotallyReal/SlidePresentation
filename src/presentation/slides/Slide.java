...
//        mainPane.setMinSize(prefWidth, prefHeight);
     ...
     *
     * Then, if this slide is on Audio mode, it starts playing the audio.
     * Finally, it makes the slide visible.
     *
     */
    ...
    /**
     * Runs the next animation of this slide. The order of the methods is by
     * their audioStart
     *
...
//        }
        ...
        /**
         * Runs the method from time millis. If millis is nonpositive, runs from
         * the start. If millis>total duration, than jumps to the end of the
         * animation. Otherwise just jump to millis and start the animation from
         * there.
         *
         * The return type of the method should only be void or a type of
         * animation. In the case it is an animation, it is automatically
         * played.
         *
         * Remark: should update so that the play would fit the narration.
         *
         * @param method the method to be run
         * @param millis the time in millis from which to start the animation.
         * use millis=0 to run from the start.
         */
...
//        anim.play();
     ...
     * minimum between the end of the markerEnd (if exists), the marker
     * beginning of the next marker (if exists) and the end of the audio.
     *
     * @param markerName
     * @return
     */
    ...
    /**
     * Leave this function here to save the markers from old version slides.
     * Once this is done, delete this function.
     *
     * @param url
     * @param markerSets
     */
...
//            }
    ...
    /**
     * Returns the subobject with the specified name. The default is to return
     * the public field with that name, and null if there isn't such (or if
     * there is and the field itself is null)
     *
     * @param name
     ...
     * aren't any. An implementing class should override this method. NOTE:
     * static methods cannot really be override, only hidden. So when calling
     * this method make sure to do it with the subclass, and not the superclass.
     * A default implementation is given by getSubObject(Class) returning all
     * the AnimationNode and Node public fields in the class.
     *
     * @return
     */
...
//        return AnimationNode.getSubobjects(cls);
    ...
    /**
     * return list of object data for all AnimationNode and Node public fields
     * in the specified class. If a field is both an AnimationNode and a Node,
     * that it is added once only as an Animation node
     *
     *
     * @param cls
     * @return
    ...
    /**
     * Returns a list of possible animation methods and null if there aren't
     * any. An implementing class should override this method. NOTE: static
     * methods cannot really be override, only hidden. So when calling this
     * method make sure to do it with the subclass, and not the superclass. A
     * default implementation is given by getAnimationList(Class) returning all
     * the methods which has Animation (or subclass) as a return type;
     *
     * @return
     */
